...
/* This version of `_getopt' appears to the caller like standard Unix `_getopt'
...
/* Describe how to deal with options that follow non-option ARGV-elements.
   ...
   `--' can cause `_getopt' to return EOF with `optind' != ARGC.
*/
...
/* char *getenv (); FB: this is not a proper declaration, getenv is already
   in stdlib anyway */
...
/* Scan elements of ARGV (whose length is ARGC) for option characters
   ...
   (aside from the initial '-') are option characters.  If `_getopt'
   is called repeatedly, it returns successively each of the option characters
   from each of the option elements.

   If `_getopt' finds another option character, it returns that character,
   updating `optind' and `nextchar' so that the next call to `_getopt' can
   resume the scan with the following option character or ARGV-element.

   If there are no more option characters, `_getopt' returns `EOF'.
   ...
   When `_getopt' finds a long-named option, it returns 0 if that option's
