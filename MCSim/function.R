...
makemcsim <- function(model, deSolve = F, dir = "MCSim"){
  ...
  if(file.exists(model)) { # Move model file from working directory to MCSim folder
    invisible(file.copy(from = paste0(getwd(),"/", model), to = paste0(getwd(),"/MCSim/", model)))
    invisible(file.remove(model))
    message(paste0("* The '", model, "' has been moved to MCSim folder."))
  }
...
mcsim <- function(model, input, dir = "MCSim", parallel = F){
  ...
  if(file.exists(input)) {
    invisible(file.copy(from = paste0(getwd(),"/", input), to = paste0(getwd(),"/MCSim/", input)))
